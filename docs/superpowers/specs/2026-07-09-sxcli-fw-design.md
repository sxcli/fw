...
| Generic provider servicification | a `fw.RegisterProvider[P engine.Provider](id, alias, factory)` helper collapsing every format-wrapper package (configfmt/yaml's shape) to an ID const and one init line — Provides and metadata prebaked. MUST stay a helper the format package CALLS: the id keeps coming from the caller, rooted in its import path, so the identity model and sxcli-vet apply unchanged; a central format registry would be the blank-import pattern back through the pantry window |
